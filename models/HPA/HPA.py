...
import time
import cv2
import models.HPA.resnet as models
import models.HPA.vgg as vgg_models
from models.HPA.ASPP import ASPP
from models.HPA.atten import Feature_Reweighting
from util.util import min_max_normalize_channels_hw


def get_train_val_set(data_set, split):
    if data_set == 'pascal':
        if split == 3:
...
class OneModel(nn.Module):
    def __init__(self, cls_type='Base', split=0,shot=1):
        ...
        if self.vgg:
            ...
            low_dim_dict = {'layer0': 64,
                            ...
                            'layer3': 512,
                            'layer4': 512}
            self.low_fea_id = 'layer2'[-1]
            low_fea_dim = low_dim_dict['layer2']
        else:
            fea_dim = 1024 + 512       
        ...
        self.Global_Pros_2 = np.load('D:/IMSN-YHM/HPA-main/prototypes/{}/{}/stage2.npy'.format(self.dataset, arch_str))  # 512-d
        self.Global_Pros_3 = np.load('D:/IMSN-YHM/HPA-main/prototypes/{}/{}/stage3.npy'.format(self.dataset, arch_str))  # 1024-d

        if self.dataset == 'pascal' or self.dataset == 'coco':
            self.train_set, self.val_set = get_train_val_set(dataset, split)
            self.Global_Pros_2 = self.Global_Pros_2[np.array(self.train_set)]
            self.Global_Pros_3 = self.Global_Pros_3[np.array(self.train_set)]
        self.Global_Pros_2 = torch.nn.Parameter(torch.from_numpy(self.Global_Pros_2), requires_grad=False)
        self.Global_Pros_3 = torch.nn.Parameter(torch.from_numpy(self.Global_Pros_3), requires_grad=False)
...
if __name__ =="__main__":
    model = OneModel(shot=5).cuda()
    query_img = torch.rand(1, 3, 256, 256).cuda()
    support_img = torch.rand(1, 5, 3, 256, 256).cuda()
    support_mask = torch.rand(1, 5, 256, 256).cuda()
    query_mask = torch.rand(1, 256, 256).cuda()
    output = model(query_img, support_img, support_mask, query_mask, cat_idx=[[torch.tensor(1).cuda()]])
