...
from models.base.feature import extract_feat_vgg, extract_feat_res
from models.base.correlation import Correlation
from models.learner import HPNLearner
...
class HypercorrSqueezeNetwork(nn.Module):
    ...
    def predict_mask(self, que_img, pos_img, pos_mask):
        logit_mask_agg = self(que_img, pos_img, pos_mask)
        logit_mask_agg = F.interpolate(logit_mask_agg, (256, 256), mode='bilinear', align_corners=True)
        logit_mask_agg = logit_mask_agg.argmax(dim=1).clone()
...
if __name__ == '__main__':
    model = HypercorrSqueezeNetwork(backbone='vgg16', use_original_imgsize=False).cuda()
