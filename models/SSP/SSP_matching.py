import models.SSP.resnet as resnet
...
class SSP_MatchingNet(nn.Module):
    ...
    def forward(self, img_s_list, mask_s_list, img_q, mask_q):
        img_s_list = img_s_list.unsqueeze(1)
    ...
    def loss(self, img_s_list, mask_s_list, img_q, mask_q):
        criterion = torch.nn.CrossEntropyLoss(ignore_index=255)
        out_ls = self(img_s_list, mask_s_list, img_q, mask_q)

        #mask_s = torch.cat(mask_s_list, dim=0)
        mask_s = mask_s_list.squeeze(1)
        mask_q= mask_q.squeeze(1)
...
if __name__ =="__main__":
    ...
    support_image = torch.rand(1, 3, 256, 256)
    support_mask = torch.ones(1, 1, 256, 256)
    qury_image = torch.rand(1, 3, 256, 256)
    qury_mask = torch.rand(1, 1, 256, 256)

    output = model(support_image, support_mask, qury_image, qury_mask)
